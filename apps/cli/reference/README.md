...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default. Use `dsh web --host 0.0.0.0 --port 3080` for direct IP access; this enables `dsh-host-web-auth`, which reads the login password from the `DSH_WEB_PASSWORD` credential reference. The built-in deployment is plain HTTP, so use a trusted network or an HTTPS reverse proxy for public exposure. `--trusted-host` adds named authorities accepted by the `/api` browser-trust fence.
