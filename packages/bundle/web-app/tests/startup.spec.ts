...
describe('web command-line provider', () => {
  ...
  it('accepts all-interfaces binding for authenticated deployments', async () => {
  ...
  it('rejects an unsupported host before the consumer activates', async () => {
    const { values, observed } = await bootProvider(['--host', 'example.invalid'])
